...
from mpmath import gamma, sin, pi, zeta, mp
...
t_values = np.linspace(0, 50, 5000)  # Reduced range to prevent overflow
...
# Normalize the real and imaginary parts before taking dot products
norm_real_amplified = rotated_real_parts_amplified / np.linalg.norm(rotated_real_parts_amplified)
norm_imag_amplified = rotated_imag_parts_amplified / np.linalg.norm(rotated_imag_parts_amplified)
norm_real_reflected = rotated_real_parts_reflected / np.linalg.norm(rotated_real_parts_reflected)
norm_imag_reflected = rotated_imag_parts_reflected / np.linalg.norm(rotated_imag_parts_reflected)

# Compute the dot product between the real and imaginary parts of s = 0.5 + it
dot_product_real_imag_amplified = np.dot(norm_real_amplified, norm_imag_reflected)

# Compute the dot product between the real and imaginary parts of 1 - s = 0.5 - it
dot_product_real_imag_reflected = np.dot(norm_real_reflected, norm_imag_amplified)

# Print the dot products
print(f"Dot product (s = 0.5 + it): {dot_product_real_imag_amplified}")
print(f"Dot product (1 - s = 0.5 - it): {dot_product_real_imag_reflected}")
