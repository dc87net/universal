...
real_vals = np.linspace(-scaleX, scaleX, sampleRate)
imag_vals = np.linspace(-scaleY, scaleY, sampleRate)
...
c0 = axs[0].contourf(z_real, z_imag, mag_expression_2, levels=50, cmap='plasma')
axs[0].set_title('|2^(z_conjugate) * π^(-z) * gamma(z)|')
axs[0].set_xlabel('Re(z)')
axs[0].set_ylabel('Im(z)')
